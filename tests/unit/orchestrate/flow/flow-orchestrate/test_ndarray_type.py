...
def linear_flow():
    f = (
        Flow()
        .add(output_array_type='list')
        .add(uses=ListInExec, output_array_type='numpy')
        .add(uses=NparrayInEec)
    )
    return f
...
def test_array_conversion(linear_flow):
    ...
    for doc in docs:
        doc.embedding = np.random.randn(5)
        doc.tensor = np.random.randn(3, 3)
