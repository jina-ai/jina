...
ENV JINA_BUILD_BASE_DEP="python3-grpcio" \
    JINA_BUILD_DEVEL_DEP="ruby-dev build-essential gcc libc-dev python3-gevent libmagic1" \
...
RUN apt-get update && apt-get install --no-install-recommends -y $JINA_BUILD_BASE_DEP && \
    if [ -n "$INSTALL_DEV" ]; then apt-get install --no-install-recommends -y $JINA_BUILD_DEVEL_DEP; gem install fluentd --no-doc; fi && \
