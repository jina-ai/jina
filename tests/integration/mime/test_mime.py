...
def inputs2(pattern=os.path.join(cur_dir, '../*.*')):
    for g in glob.glob(pattern, recursive=True)[:num_docs]:
        yield g
...
def test_any_file(mocker, restful):
    ...
    with f:
        f.index(inputs=inputs2, on_done=response_mock)
    response_mock.assert_called()
...
def test_pathURI2Buffer(mocker, restful):
    ...
    with f:
        f.index(inputs=inputs3, on_done=response_mock)
    response_mock.assert_called()
