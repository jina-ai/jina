...
class BaseExecutor(JAMLCompatible, metaclass=ExecutorType):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if __dry_run_endpoint__ not in self.requests:
            self.requests[__dry_run_endpoint__] = self._dry_run_func.__name__
        else:
            self.logger.warning(
        ...
        if type(self) == BaseExecutor:
            self.requests[__default_endpoint__] = self._dry_run_func.__name__
    ...
    def _add_requests(self, _requests: Optional[Dict]):
        if _requests:
            func_names = {f_name: e for e, f_name in self.requests.items()}
            for endpoint, func in _requests.items():
                ...
                if callable(_func):
                    ...
                    self.requests[endpoint] = _func.__name__
                    ...
                    self.requests[endpoint] = _func.fn.__name__
                else:
                    raise TypeError(
    ...
    def __call__(self, req_endpoint: str, **kwargs):
        ...
        if req_endpoint in self.requests:
            return getattr(self.__class__, self.requests[req_endpoint])(
                self, **kwargs
            )  # unbound method, self is required
        elif __default_endpoint__ in self.requests:
            return getattr(self.__class__, self.requests[__default_endpoint__])(
                self, **kwargs
            )  # unbound method, self is required
    ...
    async def __acall_endpoint__(
        ...
        func = getattr(self.__class__, self.requests[req_endpoint])
