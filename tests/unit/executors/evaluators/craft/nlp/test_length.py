...
def test_length_evaluator(doc, gt, expected):
    ...
    assert evaluator.evaluate(predicton=doc, desired=gt) == expected
...
def test_cosine_evaluator_average():
    ...
    evaluator = TextLengthEvaluator()
    assert evaluator.evaluate(predicton=doc_content[0], desired=gt_content[0]) == 1.0
    assert evaluator.evaluate(predicton=doc_content[1], desired=gt_content[1]) == 0.0
    assert evaluator.evaluate(predicton=doc_content[2], desired=gt_content[2]) == 0.0
