...
def test_incremental_indexing_vec_indexers(tmpdir):
...
def test_incremental_indexing_sequential_indexers(tmpdir):
    ...
    f = (Flow()
         .add(uses=os.path.join(cur_dir, 'vectorindexer.yml'), shards=1, name='vec_idx')
         .add(uses=os.path.join(cur_dir, 'docindexer.yml'), shards=1, name='doc_idx'))
