...
def test_recall_evaluator(eval_at, expected):
    ...
    assert (
        evaluator.evaluate(matches_ids=matches_ids, groundtruth_ids=groundtruth_ids)
        == expected
    )
...
def test_recall_evaluator_average(eval_at, expected_first):
    ...
    assert (
        evaluator.evaluate(
            matches_ids=matches_ids[0], groundtruth_ids=groundtruth_ids[0]
        )
        == expected_first
    )
    assert (
        evaluator.evaluate(
            matches_ids=matches_ids[1], groundtruth_ids=groundtruth_ids[1]
        )
        == 0.0
    )
    assert (
        evaluator.evaluate(
            matches_ids=matches_ids[2], groundtruth_ids=groundtruth_ids[2]
        )
        == 0.0
    )
