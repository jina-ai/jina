...
def test_hub_list(monkeypatch):
    monkeypatch.setenv('JINA_DB_HOSTNAME', "sample_host")
    monkeypatch.setenv('JINA_DB_USERNAME', "sample_user")
    monkeypatch.setenv('JINA_DB_PASSWORD', "sample_password")
    monkeypatch.setenv('JINA_DB_NAME', "sample_db")
    monkeypatch.setenv('JINA_DB_COLLECTION', "sample_collection")

    args = set_hub_list_parser().parse_args([ '--remote', '--kind', 'encoder', '--keywords', ['sklearn', 'numeric']])
    executors = HubIO(args).list()

    expected_executors = ['jinahub/pod.encoder.randomsparseencoder', 'jinahub/pod.encoder.dummy_mwu_encoder']
