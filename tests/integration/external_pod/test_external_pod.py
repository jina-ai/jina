...
@pytest.mark.parametrize('num_replicas', [1], indirect=True)
@pytest.mark.parametrize('num_parallel', [1], indirect=True)
def test_external_pod_shared(
    ...
    with external_pod_shared:
        ...
        with flow1:
            with flow2:
                flow1.index(inputs=input_docs)
                flow2.index(inputs=input_docs)
