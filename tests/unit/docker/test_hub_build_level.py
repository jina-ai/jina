...
def test_hub_build_level_pass(monkeypatch, mock_load_config):
    ...
    p_names, failed_levels = HubIO._test_build("jinahub/pod.dummy_mwu_encoder", BuildTestLevel.EXECUTOR, "sample/yaml.yaml", True)
...
def test_hub_build_level_fail(monkeypatch, mock_load_config):
    ...
    p_names, failed_levels = HubIO(args)._test_build("jinahub/pod.dummy_mwu_encoder", BuildTestLevel.FLOW, 'sampleconfig/yaml', True)
    assert expected_failed_levels == failed_levels
