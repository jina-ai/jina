...
class ContainerStore(BaseStore):
    ...
    def _uri(self, port: int) -> str:
        ...
        NOTE: JinaD (running inside a container) needs to access other containers via dockerhost/localhost.
        :param port: mini jinad port
        :return: uri for partial-daemon
        """

        from .. import __self__, jinad_args

        self._logger.critical(f'jinad_args.os {jinad_args.os}')
        if not os.path.exists('/.dockerenv'):
            # Checks if we actually are in docker (needed for unit tests). If not docker, use localhost.
            host = __default_host__
        elif jinad_args.os == OSOptions.LINUX:
            # Linux: connect to partial-daemon via localhost if net_mode is host
            # else, connect via dockerhost (this would only work if we start jinad passing extra_hosts)
            host = __default_host__ if __self__.network == 'host' else __docker_host__
        elif jinad_args.os in [OSOptions.MAC, OSOptions.WSL]:
            # Mac/WSL: __docker_host__ is accessible
