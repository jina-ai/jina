...
class TestWebsocketGatewayTracing(InstrumentationTestBase):
    def setUp(self):
        super().setUp()
    ...
    def test_http_span_attributes_with_executor(self):
        f = Flow(
            protocol='websocket', opentelemetry_tracing=True, opentelemetry_metrics=True
        ...
        with f:
            ...
            # There are 6 SpanKind.INTERNAL spans created for
            # a. / websocket receive (3)
            # b. / websocket send (2)
            # c. dummy (1) span created by the Executor
            self.assertEqual(6, len(internal_spans))
            ...
            self.assertEqual(16, len(self.get_finished_spans()))

            self.assertGreater(
                len(
                    TestWebsocketGatewayTracing.custom_metric_exporter.get_docs_count_data_points()
                ),
                0,
            )
