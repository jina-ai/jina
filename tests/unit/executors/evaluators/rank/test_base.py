...
def test_base_ranking_evalutor():
    ...
    with open(resource_filename('jina',
                            '/'.join(('resources', 'executors.requests.BaseRankingEvaluator.yml')))) as fp:
    ...
    assert default_eval_driver.fields != actual_eval_driver.fields
    # default value for fields in RankEvaluateDriver is overwritten by `executors.requests.BaseRankingEvaluator.yml`
    assert actual_eval_driver.fields == config_from_resources['on']['SearchRequest']['drivers'][-1].fields
