...
def document_generator(content_same, num_docs, num_chunks):
    chunk_id = num_docs
...
def test_delete_vector(config, mocker, flow_file, content_same):
    ...
    with Flow.load_config(flow_file) as index_flow:
        index_flow.index(
            input_fn=document_generator(content_same=content_same, num_docs=num_docs, num_chunks=num_chunks))
    validate_index_size(50)

    mock = mocker.Mock()
    with Flow.load_config(flow_file) as search_flow:
        search_flow.search(
            input_fn=document_generator(content_same=content_same, num_docs=num_searches, num_chunks=num_chunks),
