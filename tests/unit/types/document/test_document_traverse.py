...
def test_traverse_type(doc_req):
    ...
    assert isinstance(list(ds)[0], DocumentList)
...
def test_docuset_traverse_over_iterator_HACKY():
    # HACKY USAGE DO NOT RECOMMEND: can also traverse over "runtime"-DocumentList
    ds = DocumentList(random_docs(num_docs, num_chunks_per_doc)).traverse(['r'])
    assert len(list(list(ds)[0])) == num_docs

    ds = DocumentList(random_docs(num_docs, num_chunks_per_doc)).traverse(['c'])
    ds = list(ds)
    assert len(ds) == num_docs
    assert len(ds[0]) == num_chunks_per_doc
...
def test_docuset_traverse_over_iterator_CAVEAT():
    ...
    ds = DocumentList(random_docs(num_docs, num_chunks_per_doc)).traverse(['r', 'c'])
    ...
    ds = DocumentList(random_docs(num_docs, num_chunks_per_doc)).traverse(['c', 'r'])
