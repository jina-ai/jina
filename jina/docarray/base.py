...
class BaseProtoType:
    ...
    _PbType: Type['Message'] = None

    def __init__(
        self,
        obj: Optional[Union[bytes, 'BaseProtoType', 'Message']] = None,
        copy: bool = False,
    ):
        self._pb_body = None

        if isinstance(obj, BaseProtoType):
            if copy:
                self._pb_body = self._PbType()
                self.CopyFrom(obj)
            else:
                self._pb_body = obj._pb_body
        elif isinstance(obj, self._PbType):
            if copy:
                self._pb_body = self._PbType()
    ...
    def attributes(
        ...
        if include_proto_fields:
            support_keys = [
                k
                for k, v in vars(cls._PbType).items()
