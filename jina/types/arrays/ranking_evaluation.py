...
class RankingEvaluationMixin(ABC):
    ...
    class _DocGroundtruthPair:
        ...
        def chunks(self):
            ...
            for doc, groundtruth in zip(self.doc.chunks, self.groundtruth.chunks):
                pairs.append(
                    RankingEvaluationMixin.DocGroundtruthPair(doc, groundtruth)
                )
            return RankingEvaluationMixin._DocGroundtruthArray(pairs)

    class _DocGroundtruthArray(TraversableSequence):
    ...
    def evaluate(
        self,
        groundtruth: AbstractDocumentArray,
        ...
        **kwargs,
        ...
        :param groundtruth: The groundtruth `DocumentArray` that the `DocumentArray` compares to. It must have the
            same structure.
        :param metrics: The name of the metric, or multiple metrics to be computed
        :param attribute_fields: The list of attribute fields needed for each of the metrics to be computed. For instance,
            for `precision` one may want to compare `tags__id`
        :param eval_at: The threshold at which the metrics arecomputed. Useful to compute metrics such as precision@1..2.. etc...
        :param evaluation_names: If provided, the results of the metrics computation will be stored in the `evaluations`
         field of each Document. If not provided, the name will be computed based on the metrics name.
        :param traversal_paths: The traversal path that will be applied to both `self` and `groundtruth`
         in order to compute the ranking evaluation. Useful to evaluate solutions returning matches for `chunks` and other
         levels
