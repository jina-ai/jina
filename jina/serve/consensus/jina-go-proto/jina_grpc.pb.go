...
func (c *jinaSingleDocumentRequestRPCClient) StreamDoc(ctx context.Context, in *SingleDocumentRequestProto, opts ...grpc.CallOption) (JinaSingleDocumentRequestRPC_StreamDocClient, error) {
	stream, err := c.cc.NewStream(ctx, &JinaSingleDocumentRequestRPC_ServiceDesc.Streams[0], "/jina.JinaSingleDocumentRequestRPC/stream_doc", opts...)
	if err != nil {
		return nil, err
	}
...
// JinaSingletonRPCClient is the client API for JinaSingletonRPC service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type JinaSingletonRPCClient interface {
	CallSingle(ctx context.Context, opts ...grpc.CallOption) (JinaSingletonRPC_CallSingleClient, error)
}

type jinaSingletonRPCClient struct {
	cc grpc.ClientConnInterface
}

func NewJinaSingletonRPCClient(cc grpc.ClientConnInterface) JinaSingletonRPCClient {
	return &jinaSingletonRPCClient{cc}
}

func (c *jinaSingletonRPCClient) CallSingle(ctx context.Context, opts ...grpc.CallOption) (JinaSingletonRPC_CallSingleClient, error) {
	stream, err := c.cc.NewStream(ctx, &JinaSingletonRPC_ServiceDesc.Streams[0], "/jina.JinaSingletonRPC/CallSingle", opts...)
	if err != nil {
		return nil, err
	}
	x := &jinaSingletonRPCCallSingleClient{stream}
	return x, nil
}

type JinaSingletonRPC_CallSingleClient interface {
	Send(*SingleDocumentRequestProto) error
	Recv() (*SingleDocumentRequestProto, error)
	grpc.ClientStream
}

type jinaSingletonRPCCallSingleClient struct {
	grpc.ClientStream
}

func (x *jinaSingletonRPCCallSingleClient) Send(m *SingleDocumentRequestProto) error {
	return x.ClientStream.SendMsg(m)
}

func (x *jinaSingletonRPCCallSingleClient) Recv() (*SingleDocumentRequestProto, error) {
	m := new(SingleDocumentRequestProto)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
// JinaSingletonRPCServer is the server API for JinaSingletonRPC service.
// All implementations must embed UnimplementedJinaSingletonRPCServer
// for forward compatibility
type JinaSingletonRPCServer interface {
	CallSingle(JinaSingletonRPC_CallSingleServer) error
	mustEmbedUnimplementedJinaSingletonRPCServer()
}

// UnimplementedJinaSingletonRPCServer must be embedded to have forward compatible implementations.
type UnimplementedJinaSingletonRPCServer struct {
}

func (UnimplementedJinaSingletonRPCServer) CallSingle(JinaSingletonRPC_CallSingleServer) error {
	return status.Errorf(codes.Unimplemented, "method CallSingle not implemented")
}
func (UnimplementedJinaSingletonRPCServer) mustEmbedUnimplementedJinaSingletonRPCServer() {}

// UnsafeJinaSingletonRPCServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to JinaSingletonRPCServer will
// result in compilation errors.
type UnsafeJinaSingletonRPCServer interface {
	mustEmbedUnimplementedJinaSingletonRPCServer()
}

func RegisterJinaSingletonRPCServer(s grpc.ServiceRegistrar, srv JinaSingletonRPCServer) {
	s.RegisterService(&JinaSingletonRPC_ServiceDesc, srv)
}

func _JinaSingletonRPC_CallSingle_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(JinaSingletonRPCServer).CallSingle(&jinaSingletonRPCCallSingleServer{stream})
}

type JinaSingletonRPC_CallSingleServer interface {
	Send(*SingleDocumentRequestProto) error
	Recv() (*SingleDocumentRequestProto, error)
	grpc.ServerStream
}

type jinaSingletonRPCCallSingleServer struct {
	grpc.ServerStream
}

func (x *jinaSingletonRPCCallSingleServer) Send(m *SingleDocumentRequestProto) error {
	return x.ServerStream.SendMsg(m)
}

func (x *jinaSingletonRPCCallSingleServer) Recv() (*SingleDocumentRequestProto, error) {
	m := new(SingleDocumentRequestProto)
...
// JinaSingletonRPC_ServiceDesc is the grpc.ServiceDesc for JinaSingletonRPC service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var JinaSingletonRPC_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "jina.JinaSingletonRPC",
	HandlerType: (*JinaSingletonRPCServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "CallSingle",
			Handler:       _JinaSingletonRPC_CallSingle_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
