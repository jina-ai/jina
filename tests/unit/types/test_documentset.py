...
@pytest.fixture
def doc():
    return list(random_docs(num_docs=1, chunks_per_doc=1))[0].chunks[0]

@pytest.fixture
def docs():
    return list(random_docs(num_docs=1, chunks_per_doc=5))[0].chunks

@pytest.fixture
def docset(docs):
    return DocumentSet(docs)

def test_docset_length(docset, docs):
    assert len(docs) == len(docset) == 5 # 5 is the number of chunks

def test_add(doc, docset):
    print(doc)
    # docset.append(doc)
    # assert docset[-1] == doc
