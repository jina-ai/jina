...
def test_input_lines_with_jsonlines_file(size, sampling_rate):
    result = list(from_lines(filepath='tests/unit/clients/python/docs.jsonlines', size=size, sampling_rate=sampling_rate))
...
def test_input_lines_with_jsonslines(size, sampling_rate):
    with open('tests/unit/clients/python/docs.jsonlines') as fp:
        lines = fp.readlines()
    result = list(from_lines(lines=lines, line_format='json', size=size, sampling_rate=sampling_rate))
    assert len(result) == size if size is not None else 2
    if sampling_rate is None:
        assert result[0].text == "a"
        if size is None:
