...
async def test_connection_pool(mocker, monkeypatch):
    ...
    results = pool.send_request(
        request=ControlRequest(command='STATUS'), deployment='encoder', head=False
    )
    ...
    results = pool.send_request(
        request=ControlRequest(command='STATUS'), deployment='indexer', head=False
    )
    ...
    pool.add_connection(
        deployment='encoder', head=False, address='1.1.1.3:53', shard_id=1
    )
    ...
    assert await pool.remove_connection(
        deployment='encoder', head=False, address='1.1.1.2:53', shard_id=0
    )
    ...
    results = pool.send_request(
        request=ControlRequest(command='STATUS'), deployment='encoder', head=True
    )
    assert len(results) == 0
    assert send_mock.call_count == 10
    ...
    assert await pool.remove_connection(
        deployment='encoder', head=True, address='1.1.1.10:53'
    )
    ...
    assert await pool.remove_connection(
        deployment='encoder', head=False, address='1.1.1.1:53'
    )
    assert await pool.remove_connection(
        deployment='encoder', head=False, address='1.1.1.4:53'
    )
