...
class WebSocketBaseClient(BaseClient):
    async def _get_results(self,
        ...
        try:
            ...
            async with websockets.connect(f'ws://{client_info}/stream/{tname}') as websocket:
                ...
                with ProgressBar(task_name=tname) as p_bar, TimeContext(tname):
                    for next_request in req_iter:
                        ...
                        # Traverse through the websocket to recv messages, instead of doing `await websocket.recv()`
                        ...
                        async for response in websocket:
                            if response == b'done':
                                self.logger.debug('received done from the server. proceeding to next request')
                                break
