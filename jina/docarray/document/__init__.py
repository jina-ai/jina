...
from ..simple import StructView, ScoreMapView

if TYPE_CHECKING:
    from ..ndarray import ArrayType
    from ..simple import ScoreView
...
class Document(AllMixins, BaseProtoType):
    """
    ...
    def scores(self):
        ...
        """
        return ScoreMapView(self._pb_body.scores)

    @scores.setter
    def scores(
        self,
        value: Dict[str, Union['ScoreView', float, np.generic]],
    ):
        ...
        scores = ScoreMapView(self._pb_body.scores)
        for k, v in value.items():
            scores[k] = v

    @property
    def evaluations(self) -> ScoreMapView:
        """Return the evaluations of the document.

        :return: the evaluations attached to this document as `:class:NamedScoreMapping`
        """
        return ScoreMapView(self._pb_body.evaluations)
    ...
    def evaluations(
        ...
        if value is None:
            ...
            return
        scores = ScoreMapView(self._pb_body.evaluations)
        for k, v in value.items():
            scores[k] = v
