...
def mixin_protocol_parser(parser, pod_type = 'gateway'):
    """Add the arguments for the protocol to the gateway parser

    :param parser: the parser configure
    """

    from jina.enums import GatewayProtocolType, WorkerProtocolType
    if pod_type == 'gateway':
        help = f'Communication protocol of the server exposed by the Gateway. This can be a single value or a list of ' \
               f'protocols, depending on your chosen Gateway. Choose the convenient protocols' \
               f' from: {[protocol.to_string() for protocol in list(GatewayProtocolType)]}.',
        type = GatewayProtocolType.from_string
        choices = list(GatewayProtocolType),
        default = [GatewayProtocolType.GRPC]
    else:
        help = f'Communication protocol of the server exposed by the Executor. This can be a single value or a list ' \
               f'of protocols. Choose the convenient protocols' \
               f' from: {[protocol.to_string() for protocol in list(WorkerProtocolType)]}. '
        type = WorkerProtocolType.from_string
        choices = list(WorkerProtocolType),
        default = [WorkerProtocolType.GRPC]
...
def mixin_http_parser(arg_group):
    """Add the options to rest server
    ...
    arg_group.add_argument(
        ...
        type=str,
        help='''
        the path to the certificate file if using HTTP protocol
        ''',
        dest='ssl_certfile',
    )
    ...
    arg_group.add_argument(
        ...
        type=str,
        help='''
        the path to the key file if using HTTP protocol
        ''',
        dest='ssl_keyfile',
    )
