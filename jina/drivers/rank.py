...
class Chunk2DocRankDriver(BaseRankDriver):
    ...
    def _apply_all(self, docs: 'DocumentSet', context_doc: 'Document', *args,
        ...
        for chunk in docs:
            query_chunk_meta[chunk.id.hash_value] = chunk.get_attrs(*self.exec.required_keys)
            for match in chunk.matches:
                match_idx.append(
                    (match.parent_id.hash_value,
                     match.id.hash_value,
                     chunk.id.hash_value,
                     match.score.value)
                )
                match_chunk_meta[match.id.hash_value] = match.get_attrs(*self.exec.required_keys)
...
class CollectMatches2DocRankDriver(BaseRankDriver):
    ...
    def _apply_all(self, docs: 'DocumentSet', context_doc: 'Document', *args,
        ...
        match_chunk_meta = {}
        for match in docs:
            query_chunk_meta[context_doc.id.hash_value] = context_doc.get_attrs(*self.exec.required_keys)
            match_idx.append((
                match.parent_id.hash_value,
                match.id.hash_value,
                context_doc.id.hash_value,
                match.score.value
            ))
            match_chunk_meta[match.id.hash_value] = match.get_attrs(*self.exec.required_keys)
...
class Matches2DocRankDriver(BaseRankDriver):
    ...
    def _apply_all(self, docs: 'DocumentSet', context_doc: 'Document', *args,
        ...
        old_match_scores = {match.id.hash_value: match.score.value for match in docs}
        match_meta = {match.id.hash_value: match.get_attrs(*self.exec.required_keys) for match in docs}
