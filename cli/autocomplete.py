...
ac_table = {
    'commands': ['--help', '--version', '--version-full', 'hello-world', 'pod', 'flow', 'gateway', 'ping', 'check',
        ...
        'pod': ['--help', '--name', '--identity', '--uses', '--py-modules', '--env', '--uses-internal', '--entrypoint',
                ...
                '--expose-public', '--dump-interval', '--read-only', '--separated-workspace', '--pea-id', '--num-part',
        ...
        'gateway': ['--help', '--name', '--identity', '--uses', '--py-modules', '--env', '--uses-internal',
                    '--entrypoint', '--pull-latest', '--volumes', '--port-in', '--port-out', '--host-in', '--host-out',
                    '--socket-in', '--socket-out', '--port-ctrl', '--ctrl-with-ipc', '--timeout', '--timeout-ctrl',
                    '--timeout-ready', '--expose-public', '--dump-interval', '--read-only', '--separated-workspace',
                    '--pea-id', '--num-part', '--role', '--skip-on-error', '--memory-hwm', '--runtime',
                    '--max-idle-time', '--daemon', '--log-config', '--log-remote', '--log-id', '--ssh-server',
                    '--ssh-keyfile', '--ssh-password', '--host', '--port-expose', '--port-grpc', '--max-message-size',
                    '--proxy', '--remote-access', '--prefetch', '--prefetch-on-recv', '--rest-api', '--check-version',
        ...
        'pea': ['--help', '--name', '--identity', '--uses', '--py-modules', '--env', '--uses-internal', '--entrypoint',
                '--pull-latest', '--volumes', '--port-in', '--port-out', '--host-in', '--host-out', '--socket-in',
                '--socket-out', '--port-ctrl', '--ctrl-with-ipc', '--timeout', '--timeout-ctrl', '--timeout-ready',
                '--expose-public', '--dump-interval', '--read-only', '--separated-workspace', '--pea-id', '--num-part',
                '--role', '--skip-on-error', '--memory-hwm', '--runtime', '--max-idle-time', '--daemon', '--log-config',
