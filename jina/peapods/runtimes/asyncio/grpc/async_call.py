...
class AsyncPrefetchCall(jina_pb2_grpc.JinaRPCServicer):
    ...
    async def Call(self, request_iterator, context):
        ...
        #     await self.zmqlet.send_message(Message(None, next_request, 'gateway', **vars(self.args)))
        #     yield await self.zmqlet.recv_message(callback=handle)
        ...
        while not (self.zmqlet.msg_sent == self.zmqlet.msg_recv != 0 and is_req_empty):
