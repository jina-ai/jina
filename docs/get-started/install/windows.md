...
Jina relies heavily on `multiprocessing` to enable scaling and distribution. Windows only supports [spawn start method for multiprocessing](https://docs.python.org/3/library/multiprocessing.html#the-spawn-and-forkserver-start-methods), which has several caveats. [Please follow the guidelines here](../../fundamentals/flow/remarks#multiprocessing-spawn).
