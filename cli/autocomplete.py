...
ac_table = {
    'commands': ['--help', '--version', '--version-full', 'hello-world', 'pod', 'flow', 'gateway', 'ping', 'check',
        ...
        'pod': ['--help', '--name', '--log-config', '--identity', '--log-id', '--port-ctrl', '--ctrl-with-ipc',
                ...
                '--read-only', '--separated-workspace', '--memory-hwm', '--skip-on-error', '--num-part',
                '--uses-internal', '--entrypoint', '--pull-latest', '--volumes', '--host', '--port-expose',
                '--port-grpc', '--port-rest', '--daemon', '--runtime-backend', '--runtime', '--runtime-cls',
                '--timeout-ready', '--env', '--expose-public', '--show-exc-info', '--pea-id', '--pea-role',
                '--uses-before', '--uses-after', '--parallel', '--shards', '--polling', '--scheduling',
        ...
        'gateway': ['--help', '--name', '--log-config', '--identity', '--log-id', '--port-ctrl', '--ctrl-with-ipc',
                    ...
                    '--dump-interval', '--read-only', '--separated-workspace', '--memory-hwm', '--skip-on-error',
                    '--num-part', '--max-message-size', '--proxy', '--prefetch', '--prefetch-on-recv', '--restful',
                    '--rest-api', '--host', '--port-expose', '--port-grpc', '--port-rest', '--daemon',
                    '--runtime-backend', '--runtime', '--runtime-cls', '--timeout-ready', '--env', '--expose-public',
                    '--show-exc-info', '--pea-id', '--pea-role'],
