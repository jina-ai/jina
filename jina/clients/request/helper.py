...
def _new_doc_from_data(data, data_type: DataInputType, request_type: RequestType, **kwargs) -> Tuple['Document', 'DataInputType']:
    ...
    if data_type == DataInputType.AUTO or data_type == DataInputType.DOCUMENT:
        if isinstance(data, Document):
            ...
            return Document(id=data), DataInputType.DOCUMENT
        try:
...
def _new_request_from_batch(_kwargs, batch, data_type, mode, queryset):
    ...
    for content in batch:
        if isinstance(content, tuple) and len(content) == 2:
            ...
            d, data_type = _new_doc_from_data(content[0], data_type, mode, **_kwargs)
            gt, _ = _new_doc_from_data(content[1], data_type, mode, **_kwargs)
            req.docs.append(d)
            req.groundtruths.append(gt)
        else:
            d, data_type = _new_doc_from_data(content, data_type, mode, **_kwargs)
            req.docs.append(d)
