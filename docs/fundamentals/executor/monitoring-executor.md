...
:class: caution
The Prometheus only based feature has been deprecated in favor of the OpenTelemetry API. Refer to {ref}`Executor Instrumentation <instrumenting-executor>` and {ref}`Flow Instrumentation <instrumenting-flow>` for the current methods on observing and instrumenting Jina.
```
