...
class Zmqlet:
    ...
    def _send_message_via(self, socket, msg, out_addr):
        info_msg = f'Sending out  {msg.envelope.request_type} '
...
class AsyncZmqlet(Zmqlet):
    ...
    async def _send_message_via(self, socket, msg, out_address):
        try:
            ...
            req_type = msg.envelope.request_type
            if req_type == 'DataRequest':
                info_msg += f'({msg.envelope.header.exec_endpoint}) - ({msg.envelope.request_id}) '
            elif req_type == 'ControlRequest':
                info_msg += f'({msg.request.command}) '
            info_msg += f'to {out_address} '
