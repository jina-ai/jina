...
class BasePea(metaclass=PeaMeta):
    ...
    def __init__(self, args: Union['argparse.Namespace', Dict]):
        ...
        if isinstance(self.args, argparse.Namespace):
            ...
            self.logger = JinaLogger(self.name, group_id=self.args.identity, **vars(self.args))
        else:
            self.logger = JinaLogger(self.name, group_id=self.args.identity)
        ...
        if self.args.uses:
            try:
                self.executor = BaseExecutor.load_config(
                    ...
                    self.args.separated_workspace, self.args.pea_id, pod_id=self.args.identity)
