...
ac_table = {
    'commands': ['--help', '--version', '--version-full', 'hello', 'pod', 'flow', 'optimizer', 'gateway', 'ping',
                 'check', 'hub', 'pea', 'client', 'export-api', 'hello-world'], 'completions': {
        ...
        'pod': ['--help', '--name', '--log-config', '--identity', '--quiet-error', '--port-ctrl', '--ctrl-with-ipc',
        ...
        'flow': ['--help', '--name', '--log-config', '--identity', '--quiet-error', '--uses', '--inspect'],
        'optimizer': ['--help', '--name', '--log-config', '--identity', '--quiet-error', '--uses', '--output-dir'],
        'gateway': ['--help', '--name', '--log-config', '--identity', '--quiet-error', '--port-ctrl',
        ...
        'pea': ['--help', '--name', '--log-config', '--identity', '--quiet-error', '--port-ctrl', '--ctrl-with-ipc',
                '--timeout-ctrl', '--ssh-server', '--ssh-keyfile', '--ssh-password', '--uses', '--py-modules',
                '--port-in', '--port-out', '--host-in', '--host-out', '--socket-in', '--socket-out', '--dump-interval',
                '--read-only', '--memory-hwm', '--on-error-strategy', '--num-part', '--uses-internal', '--entrypoint',
                '--docker-kwargs', '--pull-latest', '--volumes', '--host', '--port-expose', '--silent-remote-logs',
