...
def _list(
    ...
    if params:
        response = requests.get(url=f"{hubapi_url}{hubapi_list}", params=params)
        if (
            response.status_code == requests.codes.bad_request
            and response.text == "No docs found"
        ):
            print(
...
def _push(logger, summary: Dict = None):
    ...
    try:
        response = requests.post(
            url=f"{hubapi_url}{hubapi_push}", headers=headers, data=json.dumps(summary)
        )
