...
class RankingEvaluationDriver(BaseEvaluationDriver):
    ...
    def __init__(self, id_tag="id", *args, **kwargs):
    ...
    def _apply_all(
        self, docs_groundtruths: Iterable[DocGroundtruthPair], *args, **kwargs
    ) -> None:
        for doc_groundtruth in docs_groundtruths:
            ...
            groundtruth_ids = list(
                map(lambda x: x.tags[self.id_tag], groundtruth.matches)
            )
