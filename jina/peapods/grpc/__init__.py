...
class Grpclet(jina_pb2_grpc.JinaDataRequestRPCServicer):
    ...
    @staticmethod
    async def send_ctrl_msg(pod_address: str, command: str, timeout=1.0):
        """
        ...
        """
        stub = Grpclet._create_grpc_stub(pod_address, is_async=True)
        response = await stub.Call(ControlMessage(command), timeout=timeout)
        return response
